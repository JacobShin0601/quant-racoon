...
class RFXGBEnsembleRegimeClassifier:
    ...
    def fit(self, macro_data: pd.DataFrame, force_retrain: bool = False):
        ...
        try:
            ...
            # 라벨 인코딩 (실제 존재하는 클래스만 사용)
            unique_labels = labels_aligned.unique()
            logger.info(f"실제 라벨: {sorted(unique_labels)}")
            
            # LabelEncoder를 실제 데이터에 맞춰 재설정
            self.label_encoder = LabelEncoder()
            y = self.label_encoder.fit_transform(labels_aligned.values)
            ...
            logger.info(f"인코딩된 클래스: {sorted(set(y))} → {self.label_encoder.classes_}")
    ...
    def _extract_comprehensive_features(self, macro_data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            try:
                ...
                from src.actions.ml_regime_classifier import DynamicMLRegimeClassifier
