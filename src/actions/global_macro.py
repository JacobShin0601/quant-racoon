...
def safe_json_dump(
    data: Dict[str, Any], filepath: str, description: str = "데이터", logger=None
):
...
class GlobalMacroDataCollector:
    ...
    def __init__(
        self, session_uuid: str = None, config_path: str = "config/config_macro.json"
    ):
        ...
        self.logger.info(
            f"GlobalMacroDataCollector 초기화 - Session UUID: {self.session_uuid}"
        )
        ...
        # 설정에서 매크로 지표 심볼 정의
        config_macro_symbols = self._get_macro_symbols_from_config()
        self.sector_etfs = self._get_sector_etfs_from_config()

        # 설정에서 심볼을 가져오고, 없으면 기본값 사용
        if config_macro_symbols:
            self.macro_symbols = config_macro_symbols
            self.logger.info(
                f"설정에서 매크로 심볼 로드: {list(self.macro_symbols.keys())}"
            )
        else:
            # 기본 매크로 지표 심볼 정의 (하위 호환성)
            self.macro_symbols = {
                "SPY": "S&P 500 ETF",
            ...
            self.logger.info(
                f"기본 매크로 심볼 사용: {list(self.macro_symbols.keys())}"
            )
    ...
    def _get_macro_symbols_from_config(self) -> Dict[str, str]:
        """설정에서 매크로 심볼 가져오기"""
        try:
            data_sources = self.config.get("data_collection", {}).get(
                "data_sources", {}
            )
            macro_symbols = {}
    ...
    def _get_days_back(self, collection_type: str = "default") -> int:
        ...
        try:
            ...
            period_name = data_periods.get(
                period_key, data_periods.get("default_period", "2_years")
            )
            ...
            self.logger.info(
                f"데이터 수집 기간: {collection_type} -> {period_name} ({days_back}일)"
            )
            return days_back
    ...
    def collect_spy_data(self, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.info(
                f"SPY 데이터 수집 완료: {len(df_with_indicators)}개 데이터, 컬럼: {list(df_with_indicators.columns)}"
            )
        ...
        for symbol, description in self.macro_symbols.items():
            try:
                ...
                if df is not None and not df.empty:
                    ...
                    self.logger.info(
                        f"{symbol} ({description}) 데이터 수집 완료: {len(df_with_indicators)}개 데이터"
                    )
                else:
        ...
        for symbol, sector_name in self.sector_etfs.items():
            try:
                ...
                if df is not None and not df.empty:
                    ...
                    self.logger.info(
                        f"{symbol} ({sector_name}) 섹터 데이터 수집 완료: {len(df_with_indicators)}개 데이터"
                    )
                else:
    ...
    def collect_all_data(
        self, start_date: str = None, end_date: str = None, use_cache: bool = False
    ...
    def calculate_macro_metrics(
        ...
        try:
            ...
            if "^TNX" in macro_data and "^IRX" in macro_data:
                ...
                metrics["yield_spread_ma_20"] = (
                    metrics["yield_spread"].rolling(window=20).mean()
                )
    ...
    def _calculate_tips_spread(
        self, macro_data: Dict[str, pd.DataFrame], metrics: Dict[str, pd.Series]
    ) -> Dict[str, pd.Series]:
        """TIPS Spread 계산 (인플레이션 기대치)"""
        try:
            ...
            if "TIP" in macro_data and "TLT" in macro_data:
                ...
                metrics["tips_spread_tip_tlt_ma_50"] = (
                    metrics["tips_spread_tip_tlt"].rolling(window=50).mean()
                )
            ...
            if "VTIP" in macro_data and "LTPZ" in macro_data:
                ...
                metrics["tips_spread_short_long_ma_50"] = (
                    metrics["tips_spread_short_long"].rolling(window=50).mean()
                )
            ...
            if "SCHP" in macro_data and "TLT" in macro_data:
                ...
                metrics["tips_spread_schp_tlt_ma_50"] = (
                    metrics["tips_spread_schp_tlt"].rolling(window=50).mean()
                )
            ...
            tips_spread_columns = [
                ...
                if "tips_spread" in col and "ma_50" not in col
            ]
            if tips_spread_columns:
                tips_spread_df = pd.DataFrame(
                    {col: metrics[col] for col in tips_spread_columns}
                )
                metrics["tips_spread_composite"] = tips_spread_df.mean(axis=1)
                metrics["tips_spread_composite_ma_20"] = (
                    metrics["tips_spread_composite"].rolling(window=20).mean()
                )
                metrics["tips_spread_composite_ma_50"] = (
                    metrics["tips_spread_composite"].rolling(window=50).mean()
                )

                # 인플레이션 기대치 분위수
                metrics["inflation_expectation_percentile"] = (
                    metrics["tips_spread_composite"].rolling(window=252).rank(pct=True)
                )

            self.logger.info(
                f"TIPS Spread 계산 완료: {len([col for col in metrics.keys() if 'tips_spread' in col])}개 지표"
            )
            return metrics
    ...
    def calculate_sector_rotation(
        ...
        try:
            ...
            sector_df["sector_rank"] = sector_df[rs_columns].rank(
    ...
    def save_macro_data(
        ...
        try:
            ...
            if not spy_data.empty:
                ...
                self.logger.info(
                    f"SPY 데이터 저장 완료: {len(spy_data)}개 행, {len(spy_data.columns)}개 컬럼"
                )
            ...
            for symbol, data in sector_data.items():
                if not data.empty:
                    ...
                    self.logger.info(
                        f"{symbol} 섹터 데이터 저장 완료: {len(data)}개 행"
                    )
...
class MacroSectorAnalyzer:
    ...
    def __init__(self, data_dir: str = "data/macro", session_uuid: str = None):
        ...
        self.logger.info(
            f"MacroSectorAnalyzer 초기화 - Session UUID: {self.session_uuid}"
        )
    ...
    def analyze_macro_environment(
        ...
        try:
            ...
            if "^vix_data" in macro_data:
                ...
                analysis["vix_percentile"] = (
                    vix.iloc[-1] / vix.rolling(252).max().iloc[-1]
                ) * 100
                analysis["volatility_regime"] = (
                    "high" if vix.iloc[-1] > 25 else "normal"
                )
            ...
            if "uup_data" in macro_data:
                ...
                analysis["dollar_trend"] = (
                    "strong"
                    if dxy.iloc[-1] > dxy.rolling(50).mean().iloc[-1]
                    else "weak"
                )
            ...
            if "gld_data" in macro_data:
                ...
                analysis["gold_trend"] = (
                    "bullish"
                    if gold.iloc[-1] > gold.rolling(50).mean().iloc[-1]
                    else "bearish"
                )
            ...
            if "tlt_data" in macro_data:
                ...
                analysis["bond_trend"] = (
                    "bullish"
                    if tlt.iloc[-1] > tlt.rolling(50).mean().iloc[-1]
                    else "bearish"
                )
            ...
            if "^tnx_data" in macro_data:
                ...
                analysis["rate_trend"] = (
                    "increasing" if current_rate > rate_ma_50 else "decreasing"
                )
            else:
    ...
    def _analyze_tips_spread(
        self, macro_data: Dict[str, pd.DataFrame], analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if "TIP" in macro_data and "TLT" in macro_data:
                ...
                analysis["tips_spread_tip_tlt_ma_50"] = (
                    tip_tlt_ratio.pct_change(20).rolling(50).mean().iloc[-1]
                )
            ...
            if "VTIP" in macro_data and "LTPZ" in macro_data:
                ...
                vtip_ltpz_ratio = vtip / ltpz
                analysis["tips_spread_short_long"] = vtip_ltpz_ratio.pct_change(
                    20
                ).iloc[-1]
                analysis["tips_spread_short_long_ma_50"] = (
                    vtip_ltpz_ratio.pct_change(20).rolling(50).mean().iloc[-1]
                )
            ...
            if "SCHP" in macro_data and "TLT" in macro_data:
                ...
                tlt = tlt_data[tlt_close_col]
                schp_tlt_ratio = schp / tlt
                analysis["tips_spread_schp_tlt"] = schp_tlt_ratio.pct_change(20).iloc[
                    -1
                ]
                analysis["tips_spread_schp_tlt_ma_50"] = (
                    schp_tlt_ratio.pct_change(20).rolling(50).mean().iloc[-1]
                )
            ...
            for key in [
                "tips_spread_tip_tlt",
                "tips_spread_short_long",
                "tips_spread_schp_tlt",
            ]:
                if key in analysis:
            ...
            if tips_spread_values:
                ...
                analysis["tips_spread_composite_ma_50"] = np.mean(
                    [
                        analysis.get(f"{key}_ma_50", 0)
                        for key in [
                            "tips_spread_tip_tlt",
                            "tips_spread_short_long",
                            "tips_spread_schp_tlt",
                ...
                # 인플레이션 기대치 변화 추이
                analysis["inflation_trend"] = (
                    "increasing"
                    if analysis["tips_spread_composite"]
                    > analysis["tips_spread_composite_ma_50"]
                    else "decreasing"
                )

            self.logger.info(
                f"TIPS Spread 분석 완료: 인플레이션 기대치 = {analysis.get('inflation_expectation', 'unknown')}"
            )
            return analysis
    ...
    def analyze_sector_rotation(
        ...
        try:
            # 디버깅: 섹터 데이터 확인
            self.logger.info(f"섹터 분석 시작 - 데이터 개수: {len(sector_data)}")
            for symbol, data in sector_data.items():
                self.logger.info(
                    f"섹터 {symbol}: 데이터 크기 {data.shape}, 컬럼: {list(data.columns)}"
                )
            ...
            for symbol, data in sector_data.items():
                if not data.empty and "close" in data.columns:
                    sector_returns[symbol] = data["close"].pct_change()
                    self.logger.info(
                        f"  {symbol}: 데이터 크기 {len(data)}, 수익률 계산 완료"
                    )
                else:
                    self.logger.warning(
                        f"  {symbol}: 데이터 없음 또는 'close' 컬럼 없음"
                    )

            if not sector_returns:
                self.logger.warning("수익률 계산 가능한 섹터 데이터가 없습니다")
                return sector_analysis

            self.logger.info(f"수익률 계산 완료 - 섹터 수: {len(sector_returns)}")
            ...
            rs_columns = [col for col in sector_df.columns if col.endswith("_rs")]
            if not rs_columns:
                self.logger.warning("상대 강도 컬럼이 생성되지 않았습니다")
                return sector_analysis
            ...
            self.logger.info(
                f"상대 강도 순위 계산 완료 - 섹터 수: {len(ranked_sectors)}"
            )

            # 섹터 강도 분류
            self.logger.info(
                f"섹터 분류 시작 - ranked_sectors: {list(ranked_sectors.index)}"
            )
            for symbol in sector_data.keys():
                # ranked_sectors의 키는 'symbol_rs' 형태이므로 매칭
                rs_key = f"{symbol}_rs"
                if rs_key in ranked_sectors.index:
                    rank = ranked_sectors[rs_key]
                    total_sectors = len(ranked_sectors)

                    if rank <= total_sectors * 0.3:  # 상위 30%
                        sector_analysis[symbol] = SectorStrength.LEADING
                        self.logger.info(
                            f"  {symbol}: LEADING (순위 {rank}/{total_sectors})"
                        )
                    elif rank >= total_sectors * 0.7:  # 하위 30%
                        sector_analysis[symbol] = SectorStrength.LAGGING
                        self.logger.info(
                            f"  {symbol}: LAGGING (순위 {rank}/{total_sectors})"
                        )
                    else:
                        # 섹터 타입에 따라 분류
                        sector_type = self.sector_classification.get(symbol, {}).get(
                            "type", SectorStrength.CYCLICAL
                        )
                        sector_analysis[symbol] = sector_type
                        self.logger.info(
                            f"  {symbol}: {sector_type.value} (순위 {rank}/{total_sectors})"
                        )
                else:
                    self.logger.warning(
                        f"  {symbol}: 순위 데이터에서 찾을 수 없음 (rs_key: {rs_key})"
                    )

            self.logger.info(f"섹터 분석 완료 - 결과 개수: {len(sector_analysis)}")
            return sector_analysis
    ...
    def generate_sector_recommendations(
        self,
        market_condition: MarketCondition,
        sector_analysis: Dict[str, SectorStrength],
    ) -> Dict[str, Any]:
        ...
        if market_condition == MarketCondition.BULL_MARKET:
            ...
            recommendations["strategy"] = (
                "Momentum following - 순환적 섹터 중 선도 섹터 집중"
            )
            ...
            for symbol, strength in sector_analysis.items():
                ...
                if (
                    ...
                    and sector_type == SectorStrength.CYCLICAL
                ):
            ...
            # 경기침체 우려: 방어적 섹터 + 국채
            recommendations["strategy"] = (
                "Recession hedge - 방어적 섹터 + 국채 비중 확대"
            )
            recommendations["risk_level"] = "low"
            ...
            recommendations["strategy"] = (
                "Inflation hedge - 실물자산 + 에너지 + 원자재 집중"
            )
    ...
    def get_comprehensive_analysis(
        ...
        try:
            ...
            recommendations = self.generate_sector_recommendations(
                market_condition, sector_analysis
            )
    ...
    def _calculate_confidence(
        self, macro_analysis: Dict[str, Any], sector_analysis: Dict[str, SectorStrength]
    ) -> float:
        """매크로 분석 신뢰도 계산"""
        confidence = 0.5  # 기본값
    ...
    def save_analysis_results(
        self, analysis: MacroAnalysis, output_dir: str = "results/macro_sector_analysis"
    ):
        """매크로 분석 결과 저장"""
        try:
            # UUID 기반 디렉토리 생성
            ...
            results = {
                ...
                "sector_rotation": {
                    k: v.value for k, v in analysis.sector_rotation.items()
                },
            ...
            # JSON 파일로 저장
            with open(
                f"{session_dir}/analysis_results.json", "w", encoding="utf-8"
            ) as f:
...
class HyperparamTuner:
    ...
    def __init__(
        self, config_path: str = "config/config_macro.json", session_uuid: str = None
    ):
        self.logger = logging.getLogger(__name__)
        self.config = self._load_config(config_path)
        self.session_uuid = session_uuid or str(uuid.uuid4())
    ...
    def _calculate_pivot_points(
        self, high: pd.Series, low: pd.Series, close: pd.Series, period: int = 1
    ) -> pd.Series:
        """Pivot Points 계산"""
        pivot = (
    ...
    def _calculate_fibonacci_levels(
        self, high: pd.Series, low: pd.Series, level: float
    ) -> pd.Series:
    ...
    def _calculate_adx(
        ...
        # +DI, -DI 계산
        plus_di = (
            100 * pd.Series(plus_dm).rolling(period).mean() / tr.rolling(period).mean()
        )
        minus_di = (
            100 * pd.Series(minus_dm).rolling(period).mean() / tr.rolling(period).mean()
        )
    ...
    def _calculate_technical_scores(
        ...
        if "trend_weight" in params:
            ...
            log_returns_ma = log_returns.rolling(
                window=params.get("log_returns_ma_period", 20)
            ).mean()
            log_returns_std = log_returns.rolling(
                window=params.get("log_returns_ma_period", 20)
            ).std()
            ...
            log_return_trend = np.where(
                ~log_returns_ma.isna(),
                np.where(
                    log_returns_ma > params.get("log_returns_ma_threshold", 0.0005),
                    0.5,
                    np.where(
                        log_returns_ma
                        < -params.get("log_returns_ma_threshold", 0.0005),
                        -0.5,
                        0,
                    ),
                ),
            ...
            log_return_volatility = np.where(
                ...
                np.where(
                    log_returns_std > log_returns_std.quantile(0.7), 0.3, 0
                ),  # 높은 변동성
                0,
            )
            ...
            # 기존 지표들
            if (
                f'sma_{params.get("sma_short", 20)}' in data.columns
                and f'sma_{params.get("sma_long", 50)}' in data.columns
            ):
                ...
                if "supertrend" in data.columns:
                    ...
                    supertrend_signal = np.where(
                        valid_mask & (data[close_col] > supertrend), 0.2, -0.2
                    )

                sma_trend = np.where(
                    valid_mask, np.where(sma_short > sma_long, 0.5, -0.5), 0
                )
            ...
            if "rsi" in data.columns:
                ...
                rsi_momentum = np.where(
                    ...
                    np.where(
                        ...
                        0,
                        np.where(rsi > params.get("rsi_overbought", 70), -1, 1),
                    ),
                    0,
                )
                ...
                if "macd" in data.columns and "macd_signal" in data.columns:
                    ...
                    macd_momentum = np.where(
                        valid_mask & (macd > macd_signal), 0.5, -0.5
                    )

                stoch_momentum = 0
                if "stoch_k" in data.columns:
                    stoch_k = data["stoch_k"]
                    stoch_momentum = np.where(
                        valid_mask & (stoch_k > 80),
                        -0.3,
                        np.where(valid_mask & (stoch_k < 20), 0.3, 0),
                    )

                momentum_score = rsi_momentum + macd_momentum + stoch_momentum
            regime_scores["momentum_score"] = momentum_score * params.get(
                "momentum_weight", 0.3
            )
        ...
        if "volatility_weight" in params:
            ...
            if "atr" in data.columns and close_col in data.columns:
                ...
                atr_volatility = np.where(
                    valid_mask, np.where(atr_ratio > 0.02, 1, 0), 0
                )
                ...
                if "keltner_upper" in data.columns and "keltner_lower" in data.columns:
                    ...
                    keltner_volatility = np.where(
                        valid_mask & (keltner_width > 0.03), 0.3, 0
                    )

                volatility_score = atr_volatility + bb_volatility + keltner_volatility
            regime_scores["volatility_score"] = volatility_score * params.get(
        ...
        if "macro_weight" in params:
            ...
            if "^VIX" in data.columns:
                vix_col = "^VIX"
            elif "close" in data.columns and "^vix_data" in str(
                data.columns
            ):  # VIX 데이터의 close 컬럼
                vix_col = "close"
            ...
            if vix_col:
                ...
                valid_mask = ~vix.isna()
                vix_score = np.where(
                    valid_mask, np.where(vix > params.get("vix_threshold", 25), 1, 0), 0
                )
        ...
        if "volume_weight" in params:
            ...
            if "volume" in data.columns:
                volume_col = "volume"
            if (
                "average_volume" in data.columns
            ):  # SPY 데이터의 average_volume 컬럼 사용
            ...
            if volume_col and volume_ma_col:
                ...
                volume_ma = volume.rolling(
                    window=params.get("volume_ma_period", 20)
                ).mean()
                volume_ratio = volume / volume_ma
                volume_score = np.where(
                    valid_mask & (volume_ratio > 1.5),
            ...
            regime_scores["volume_score"] = volume_score * params.get(
                "volume_weight", 0.1
            )
        ...
        if "support_resistance_weight" in params:
            ...
            if pivot_col:
                ...
                pivot_score = np.where(
                    valid_mask & (abs(support_distance) < 0.01), 0.3, 0
                )
            ...
            if fibonacci_cols:
                for fib_col in fibonacci_cols:
                    ...
                    fib_score = np.where(
                        valid_mask & (abs(fib_distance) < 0.005), 0.2, 0
                    )  # 더 엄격한 임계값
                    sr_score += fib_score
            ...
            if "bb_upper" in data.columns and "bb_lower" in data.columns:
                ...
                bb_score = np.where(
                    valid_mask & (upper_distance < 0.01), 0.2, 0
                )  # 저항선 근처
                bb_score += np.where(
                    valid_mask & (lower_distance < 0.01), 0.2, 0
                )  # 지지선 근처
                sr_score += bb_score
            ...
            if "keltner_upper" in data.columns and "keltner_lower" in data.columns:
                ...
                keltner_upper_distance = (keltner_upper - data[close_col]) / data[
                    close_col
                ]
                keltner_lower_distance = (data[close_col] - keltner_lower) / data[
                    close_col
                ]

                keltner_score = np.where(
                    valid_mask & (keltner_upper_distance < 0.01), 0.15, 0
                )  # 저항선 근처
                keltner_score += np.where(
                    valid_mask & (keltner_lower_distance < 0.01), 0.15, 0
                )  # 지지선 근처
                sr_score += keltner_score

            regime_scores["sr_score"] = sr_score * params.get(
    ...
    def _calculate_derived_features(
        ...
        result["cci"] = self._calculate_cci(
            result[high_col], result[low_col], result[close_col], cci_period
        )

        # Williams %R
        williams_r_period = params.get("williams_r_period", 14)
        result["williams_r"] = self._calculate_williams_r(
            result[high_col], result[low_col], result[close_col], williams_r_period
        )

        # 3. 변동성 지표 (기존 + 개선)
        # ATR
        atr_period = params.get("atr_period", 14)
        result["atr"] = self._calculate_atr(
            result[high_col], result[low_col], result[close_col], atr_period
        )
        ...
        bb_upper, bb_middle, bb_lower = self._calculate_bollinger_bands(
            result[close_col], bb_period, bb_std
        )
        ...
        result["fibonacci_0.236"] = self._calculate_fibonacci_levels(
            result[high_col], result[low_col], 0.236
        )
        result["fibonacci_0.382"] = self._calculate_fibonacci_levels(
            result[high_col], result[low_col], 0.382
        )
        result["fibonacci_0.500"] = self._calculate_fibonacci_levels(
            result[high_col], result[low_col], 0.500
        )
        result["fibonacci_0.618"] = self._calculate_fibonacci_levels(
            result[high_col], result[low_col], 0.618
        )
        result["fibonacci_0.786"] = self._calculate_fibonacci_levels(
            result[high_col], result[low_col], 0.786
        )
        ...
        result["mfi"] = self._calculate_money_flow_index(
            result[high_col],
            result[low_col],
            result[close_col],
            result[volume_col],
        ...
        # Pivot Points
        pivot_period = params.get("pivot_period", 1)
        result["pivot"] = self._calculate_pivot_points(
            result[high_col], result[low_col], result[close_col], pivot_period
        )

        # Fibonacci Retracement
        fibonacci_levels = params.get("fibonacci_levels", 5)
        result["fib_23_6"] = self._calculate_fibonacci_levels(
            result[high_col], result[low_col], 0.236
        )
        result["fib_38_2"] = self._calculate_fibonacci_levels(
            result[high_col], result[low_col], 0.382
        )
        result["fib_50_0"] = self._calculate_fibonacci_levels(
            result[high_col], result[low_col], 0.500
        )
        result["fib_61_8"] = self._calculate_fibonacci_levels(
            result[high_col], result[low_col], 0.618
        )
        ...
        result["fib_38"] = self._calculate_fibonacci_levels(
            result[high_col], result[low_col], 0.382
        )
        result["fib_50"] = self._calculate_fibonacci_levels(
            result[high_col], result[low_col], 0.5
        )
        result["fib_61"] = self._calculate_fibonacci_levels(
            result[high_col], result[low_col], 0.618
        )
    ...
    def _classify_market_regime(
        ...
        if "spy_trend_weight" in params:
            ...
            if (
                ...
                sma_trend = np.where(
                    valid_mask, np.where(sma_short > sma_long, 1, -1), 0
                )
                ...
                if "supertrend" in data.columns:
                    supertrend = data["supertrend"]
                    supertrend_signal = np.where(
                        valid_mask & (data[close_col] > supertrend), 0.3, -0.3
                    )
                ...
                if close_col in data.columns:
                    ...
                    returns_score = np.where(
                        valid_mask,
                        np.where(
                            returns_5d > 0.02,
                            0.4,  # 5일 수익률 > 2%
                            np.where(returns_5d < -0.02, -0.4, 0),
                        )  # 5일 수익률 < -2%
                        + np.where(
                            returns_10d > 0.03,
                            0.3,  # 10일 수익률 > 3%
                            np.where(returns_10d < -0.03, -0.3, 0),
                        )  # 10일 수익률 < -3%
                        + np.where(
                            returns_20d > 0.05,
                            0.2,  # 20일 수익률 > 5%
                            np.where(returns_20d < -0.05, -0.2, 0),
                        ),
                        0,
                    )  # 20일 수익률 < -5%
                ...
                if (
                    ...
                    price_position_score = np.where(
                        valid_mask & (bb_position > 0.8),
                        0.2,  # 상단 20%
                        np.where(valid_mask & (bb_position < 0.2), -0.2, 0),
                    )  # 하단 20%
                ...
                spy_trend_score = (
                    sma_trend * 0.4
                    + adx_strength * 0.3
                    + supertrend_signal * 0.2
                    + returns_momentum * 0.1
                    + price_position_score * 0.1
                )

            regime_scores["spy_trend_score"] = (
                spy_trend_score * params["spy_trend_weight"]
            )
        ...
        if "momentum_weight" in params:
            ...
            if "rsi" in data.columns:
                ...
                momentum_score = (
                    rsi_momentum * 0.5 + macd_momentum * 0.3 + stoch_momentum * 0.2
                )
        ...
        if "volatility_weight" in params:
            ...
            if "atr" in data.columns and close_col in data.columns:
                ...
                volatility_score = atr_volatility + bb_volatility + keltner_volatility
            regime_scores["volatility_score"] = (
        ...
        if "macro_weight" in params:
            ...
            if "^VIX" in data.columns:
                ...
                vix_level_score = np.where(
                    valid_mask,
                    np.where(
                        vix > params.get("vix_high_threshold", 30),
                        1,  # 높은 변동성
                        np.where(
                            vix > params.get("vix_medium_threshold", 20),
                            0.5,  # 중간 변동성
                            np.where(
                                vix < params.get("vix_low_threshold", 15), -0.3, 0
                            ),
                        ),
                    ),
                    0,
                    ...
                    np.where(
                        ...
                        np.where(
                            ...
                            np.where(
                                vix_change < -0.15,
                                -0.8,  # 급격한 하락
                                np.where(vix_change < -0.05, -0.4, 0),
                            ),
                        ),
                    ),
                    0,
                ...
                if len(vix) >= 20:
                    ...
                    vix_ma_score = np.where(
                        valid_mask & (vix > vix_ma_20 * 1.2),
                        0.5,  # VIX > MA20 * 1.2
                        np.where(valid_mask & (vix < vix_ma_20 * 0.8), -0.3, 0),
                    )  # VIX < MA20 * 0.8

                vix_total_score = (
                    vix_level_score * 0.4
                    + vix_momentum_score * 0.3
                    + vix_ma_score * 0.3
                )
            ...
            if "TIPS_SPREAD" in data.columns:
                ...
                tips_level_score = np.where(
                    valid_mask,
                    np.where(
                        tips_spread > params.get("tips_high_threshold", 2.5),
                        0.8,  # 높은 인플레이션 기대
                        np.where(
                            tips_spread > params.get("tips_medium_threshold", 2.0),
                            0.4,  # 중간 인플레이션 기대
                            np.where(
                                tips_spread < params.get("tips_low_threshold", 1.5),
                                -0.4,
                                0,
                            ),
                        ),
                ...
                tips_momentum_score = np.where(
                    valid_mask,
                    np.where(
                        tips_change > 0.1,
                        0.5,  # 인플레이션 기대 상승
                        np.where(tips_change < -0.1, -0.5, 0),
                    ),
                    0,
                )  # 인플레이션 기대 하락
            ...
            if "^DXY" in data.columns:
                ...
                dxy_strength_score = np.where(
                    valid_mask & (dxy > dxy_ma_20 * 1.05),
                    0.3,  # 강한 달러
                    np.where(valid_mask & (dxy < dxy_ma_20 * 0.95), -0.3, 0),
                )  # 약한 달러
            ...
            if "GC=F" in data.columns:
                ...
                gold_trend_score = np.where(
                    valid_mask & (gold > gold_ma_20 * 1.05),
                    0.2,  # 금 상승
                    np.where(valid_mask & (gold < gold_ma_20 * 0.95), -0.2, 0),
                )  # 금 하락
            ...
            if "^TNX" in data.columns:
                ...
                treasury_score = np.where(
                    valid_mask & (treasury_10y > treasury_ma_20 * 1.1),
                    0.3,  # 금리 상승
                    np.where(
                        valid_mask & (treasury_10y < treasury_ma_20 * 0.9), -0.3, 0
                    ),
                )  # 금리 하락
        ...
        regime = np.where(
            total_score > trending_up_threshold,
            MarketRegime.TRENDING_UP.value,
            np.where(
                total_score < trending_down_threshold,
                MarketRegime.TRENDING_DOWN.value,
                np.where(
                    regime_scores["volatility_score"] > volatile_threshold,
    ...
    def _classify_market_regime_with_probabilities(
        self, data: pd.DataFrame, params: Dict[str, Any]
    ) -> Dict[str, Any]:
        """시장 상태 분류 (확률 포함)"""
        ...
        def calculate_regime_probabilities_random_forest(scores):
            ...
            features = np.column_stack(
                [
                    ...
                    regime_scores.get(
                        "macro_score", np.zeros_like(scores)
                    ),  # 매크로 점수
                    regime_scores.get(
                        "volume_score", np.zeros_like(scores)
                    ),  # 거래량 점수
                    regime_scores.get(
                        "sr_score", np.zeros_like(scores)
                    ),  # 지지/저항 점수
                    # 절댓값 특성들
                    ...
                    # 상호작용 특성들
                    regime_scores["trend_score"]
                    * regime_scores["momentum_score"],  # 트렌드-모멘텀 상호작용
                    regime_scores["trend_score"]
                    * regime_scores["volatility_score"],  # 트렌드-변동성 상호작용
                    regime_scores["momentum_score"]
                    * regime_scores["volatility_score"],  # 모멘텀-변동성 상호작용
                    regime_scores["volatility_score"]
                    * np.abs(
                        regime_scores["momentum_score"]
                    ),  # 변동성-모멘텀 절댓값 상호작용
                    # 비율 특성들
                    np.where(
                        regime_scores["volatility_score"] != 0,
                        regime_scores["momentum_score"]
                        / regime_scores["volatility_score"],
                        0,
                    ),  # 모멘텀/변동성 비율
                    np.where(
                        regime_scores["momentum_score"] != 0,
                        regime_scores["trend_score"]
                        / np.abs(regime_scores["momentum_score"]),
                        0,
                    ),  # 트렌드/모멘텀 비율
                    # 제곱 특성들 (비선형 관계 포착)
                    scores**2,  # 총점 제곱
                    ...
                    (regime_scores["volatility_score"] > volatile_threshold).astype(
                        float
                    ),  # 고변동성 여부
                    (regime_scores["trend_score"] > 0).astype(
                        float
                    ),  # 트렌드 양수 여부
                    (regime_scores["momentum_score"] > 0).astype(
                        float
                    ),  # 모멘텀 양수 여부
                    # 복합 특성들
                    np.where(
                        scores > 0, scores * regime_scores["trend_score"], 0
                    ),  # 양수 총점 * 트렌드
                    np.where(
                        scores < 0, scores * regime_scores["trend_score"], 0
                    ),  # 음수 총점 * 트렌드
                    ...
                    np.where(
                        regime_scores["volatility_score"] > 0.7, 1.0, 0.0
                    ),  # 매우 높은 변동성
                    np.where(
                        np.abs(regime_scores["momentum_score"]) > 0.6, 1.0, 0.0
                    ),  # 매우 높은 모멘텀
                    # 균형 특성들
                    np.abs(scores)
                    * regime_scores["volatility_score"],  # 총점 절댓값 * 변동성
                    regime_scores["trend_score"]
                    * np.abs(regime_scores["momentum_score"]),  # 트렌드 * 모멘텀 절댓값
                    # 매크로 관련 특성들
                    regime_scores.get("macro_score", np.zeros_like(scores))
                    * regime_scores["volatility_score"],  # 매크로 * 변동성
                    regime_scores.get("volume_score", np.zeros_like(scores))
                    * np.abs(regime_scores["momentum_score"]),  # 거래량 * 모멘텀 절댓값
                ]
            )
            ...
            def tree_based_probability(features_row, regime_type):
                ...
                if regime_type == "trending_up":
                    ...
                    conditions = [
                        ...
                        trend_momentum
                        > 0.02,  # 트렌드-모멘텀 상호작용이 양수 (임계값 더 낮춤)
                    ...
                    not_trending_up = not (
                        score > trending_up_threshold and trend > 0.2
                    )
                    not_trending_down = not (
                        score < trending_down_threshold and trend < -0.2
                    )
                    not_volatile = not (
                        volatility > volatile_threshold and abs_momentum > 0.3
                    )
            ...
            for i in range(len(scores)):
                ...
                probabilities[i, 0] = tree_based_probability(
                    features_row, "trending_up"
                )
                probabilities[i, 1] = tree_based_probability(
                    features_row, "trending_down"
                )
        ...
        # 신뢰도 계산 (확률의 최대값 기반)
        current_probs = (
            regime_probs[-1] if len(regime_probs) > 0 else [0.25, 0.25, 0.25, 0.25]
        )
    ...
    def _calculate_strategy_returns(
        self, data: pd.DataFrame, regime: pd.Series, params: Dict[str, Any]
    ) -> pd.Series:
        """전략별 수익률 계산"""
        ...
        if first_regime == MarketRegime.TRENDING_UP.value:
            ...
            position.iloc[0] = params.get("volatile_reduction", 0.5) * params.get(
                "base_position", 0.8
            )
        else:  # UNCERTAIN
            position.iloc[0] = 0
        ...
        for i in range(1, len(data)):
            ...
            if current_regime == MarketRegime.TRENDING_UP.value:
                ...
                # 포지션 크기 축소 + 단기 전략
                position.iloc[i] = params.get("volatile_reduction", 0.5) * params.get(
                    "base_position", 0.8
                )
    ...
    def _calculate_performance_metrics(
        self, strategy_returns: pd.Series, buy_hold_returns: pd.Series
    ) -> Dict[str, float]:
        """성과 지표 계산 (고도화된 버전)"""
        # 총 수익률 (단순 수익률)
        ...
        for ret in strategy_returns:
            if ret > 0:
                ...
                max_consecutive_losses = max(
                    max_consecutive_losses, abs(current_streak)
                )
        ...
        trade_frequency = (
            position_changes.sum() / len(strategy_returns)
    ...
    def objective(
        ...
        # 지표 파라미터 샘플링
        indicators = self.config.get("market_regime_classification", {}).get(
            "indicators", {}
        )
        ...
        for category, indicators_dict in indicators.items():
            for indicator, config in indicators_dict.items():
                if config["type"] == "int":
                    params[indicator] = trial.suggest_int(
                        indicator, config["min"], config["max"]
                    )
                elif config["type"] == "float":
                    params[indicator] = trial.suggest_float(
                        indicator, config["min"], config["max"]
                    )

        # 분류 가중치 샘플링
        weights = self.config.get("market_regime_classification", {}).get(
        ...
        try:
            ...
            strategy_returns = self._calculate_strategy_returns(
                data_with_features, regime, params
            )
            ...
            # 목적 함수 선택 (승률 중심으로 개선)
            objective_metric = self.config.get("optimization", {}).get(
                "objective", "win_rate_optimized"
            )
    ...
    def optimize_hyperparameters_with_data(
        self,
        spy_data: pd.DataFrame,
        macro_data: Dict[str, pd.DataFrame],
        n_trials: int = None,
        ...
        # Train/Test 분할
        train_test_split = self.config.get("optimization", {}).get(
        ...
        train_macro = {
            k: v.iloc[:split_idx] if not v.empty else v for k, v in macro_data.items()
        }
        test_macro = {
            k: v.iloc[split_idx:] if not v.empty else v for k, v in macro_data.items()
        }
        ...
        self.logger.info(
            f"Train 데이터: {len(train_spy)}개, Test 데이터: {len(test_spy)}개"
        )
        ...
        # Test 데이터에서 성과 평가
        test_performance = self._evaluate_on_test_data(
            test_spy, test_macro, best_params
        )
    ...
    def optimize_hyperparameters(
        ...
        self.logger.info(
            f"최적화 완료 - Best {self.config.get('optimization', {}).get('objective', 'sharpe_ratio')}: {best_value:.4f}"
        )
    ...
    def _evaluate_on_test_data(
        self,
        test_spy: pd.DataFrame,
        test_macro: Dict[str, pd.DataFrame],
        ...
        try:
            ...
            # 전략 수익률 계산
            strategy_returns = self._calculate_strategy_returns(
                data_with_features, regime, best_params
            )
...
class MarketRegimeValidator:
    ...
    def validate_classification_accuracy(
        self, actual_regimes: pd.Series, predicted_regimes: pd.Series
    ) -> Dict[str, Any]:
        """시장 상태 분류 정확도 검증"""
        try:
            ...
            class_report = classification_report(
                actual_regimes, predicted_regimes, output_dict=True
            )
            ...
            for regime in unique_regimes:
                ...
                if mask.sum() > 0:
                    regime_accuracy[regime] = (
                        actual_regimes[mask] == predicted_regimes[mask]
                    ).mean()
            ...
            lag_analysis = self._analyze_prediction_lag(
                actual_regimes, predicted_regimes
            )
    ...
    def _analyze_prediction_lag(
        self, actual: pd.Series, predicted: pd.Series, max_lag: int = 5
    ) -> Dict[str, Any]:
        """예측 지연 분석"""
        lag_analysis = {}
    ...
    def analyze_strategy_performance(
        ...
        try:
            ...
            strategy_drawdown = (
                strategy_cumulative / strategy_cumulative.expanding().max() - 1
            ).min()
            benchmark_drawdown = (
                benchmark_cumulative / benchmark_cumulative.expanding().max() - 1
            ).min()
            ...
            for regime in unique_regimes:
                ...
                if mask.sum() > 0:
                    ...
                    regime_sharpe = (
                        regime_strategy_returns.mean() * 252 - risk_free_rate
                    ) / regime_vol
                    regime_win_rate = (regime_strategy_returns > 0).mean()

                    # 정보 비율 계산
                    regime_information_ratio = (
                        regime_excess_return
                        / (regime_strategy_returns - regime_benchmark_returns).std()
                    )
            ...
            strategy_cvar_95 = strategy_returns[
                strategy_returns <= strategy_var_95
            ].mean()
            benchmark_cvar_95 = benchmark_returns[
                benchmark_returns <= benchmark_var_95
            ].mean()
    ...
    def _calculate_regime_effectiveness(
        ...
        for regime, performance in regime_performance.items():
            ...
            # 효과성 점수 (가중 평균)
            effectiveness_score = (
                excess_return * 0.4 + sharpe_ratio * 0.3 + information_ratio * 0.3
            )
        ...
        # 성과 순위 계산
        scores = [
            (regime, data["effectiveness_score"])
            for regime, data in effectiveness.items()
        ]
    ...
    def generate_validation_report(
        ...
        try:
            ...
            if "overall_accuracy" in validation_results:
                ...
                report.append("-" * 50)
                report.append(
                    f"전체 정확도: {validation_results['overall_accuracy']:.3f} ({validation_results['overall_accuracy']*100:.1f}%)"
                )
                ...
                report.append(
                    f"상태 변화 정확도: {validation_results['change_accuracy']:.3f}"
                )
                report.append("")
                ...
                if "regime_accuracy" in validation_results:
                    ...
                    for regime, accuracy in validation_results[
                        "regime_accuracy"
                    ].items():
                        report.append(
                            f"  {regime}: {accuracy:.3f} ({accuracy*100:.1f}%)"
                        )
                    report.append("")
                ...
                if "lag_analysis" in validation_results:
                    ...
                    for lag, data in validation_results["lag_analysis"].items():
                        report.append(
                            f"  {lag}일 지연: 상관관계={data['correlation']:.3f}, 정확도={data['accuracy']:.3f}"
                        )
                    report.append("")
            ...
            if "overall_performance" in performance_results:
                ...
                report.append("-" * 50)
                report.append(
                    f"총 수익률: {perf['total_return']:.3f} ({perf['total_return']*100:.1f}%)"
                )
                report.append(
                    f"벤치마크 수익률: {perf['benchmark_return']:.3f} ({perf['benchmark_return']*100:.1f}%)"
                )
                report.append(
                    f"초과 수익률: {perf['excess_return']:.3f} ({perf['excess_return']*100:.1f}%)"
                )
                report.append(
                    f"변동성: {perf['volatility']:.3f} ({perf['volatility']*100:.1f}%)"
                )
                report.append(f"샤프 비율: {perf['sharpe_ratio']:.3f}")
                report.append(
                    f"최대 낙폭: {perf['max_drawdown']:.3f} ({perf['max_drawdown']*100:.1f}%)"
                )
                report.append(
                    f"승률: {perf['win_rate']:.3f} ({perf['win_rate']*100:.1f}%)"
                )
                report.append(f"정보 비율: {perf['information_ratio']:.3f}")
                report.append(
                    f"VaR (95%): {perf['var_95']:.3f} ({perf['var_95']*100:.1f}%)"
                )
                report.append(
                    f"CVaR (95%): {perf['cvar_95']:.3f} ({perf['cvar_95']*100:.1f}%)"
                )
                ...
                if "regime_performance" in performance_results:
                    ...
                    for regime, regime_perf in performance_results[
                        "regime_performance"
                    ].items():
                        report.append(f"  {regime}:")
                        report.append(
                            f"    수익률: {regime_perf['total_return']:.3f} ({regime_perf['total_return']*100:.1f}%)"
                        )
                        report.append(
                            f"    초과 수익률: {regime_perf['excess_return']:.3f} ({regime_perf['excess_return']*100:.1f}%)"
                        )
                        report.append(
                            f"    샤프 비율: {regime_perf['sharpe_ratio']:.3f}"
                        )
                        report.append(
                            f"    승률: {regime_perf['win_rate']:.3f} ({regime_perf['win_rate']*100:.1f}%)"
                        )
                        report.append(
                            f"    정보 비율: {regime_perf['information_ratio']:.3f}"
                        )
                ...
                if "regime_effectiveness" in performance_results:
                    ...
                    sorted_effectiveness = sorted(
                        effectiveness.items(), key=lambda x: x[1]["performance_rank"]
                    )
                    for regime, data in sorted_effectiveness:
                        report.append(
                            f"  {data['performance_rank']}. {regime}: {data['effectiveness_score']:.3f}"
                        )
                    report.append("")
            ...
            if (
                "overall_accuracy" in validation_results
                and validation_results["overall_accuracy"] > 0.7
            ):
    ...
    def save_validation_results(
        ...
        try:
            ...
            results_file = os.path.join(
                output_dir, f"validation_results_{timestamp}.json"
            )
    ...
    def save_results(
        self, results: Dict[str, Any], output_dir: str = "results/macro_optimization"
    ):
        ...
        try:
            ...
            metadata = {
                "session_uuid": self.session_uuid,
                "optimization_objective": self.config.get("optimization", {}).get(
            ...
            metadata_success = safe_json_dump(
                metadata, f"{session_dir}/metadata.json", "메타데이터", self.logger
            )
            ...
            if "study" in results:
                try:
                    ...
                    if hasattr(results["study"], "export_data"):
                        ...
                        self.logger.info(
                            f"✅ Optuna 스터디 CSV 저장 완료: {session_dir}/optuna_study.csv"
                        )
                    else:
            ...
            if successful_files == total_files:
                self.logger.info(
                    f"✅ 모든 결과 저장 완료: {session_dir} ({successful_files}/{total_files} 파일)"
                )
            else:
                self.logger.warning(
                    f"⚠️ 일부 결과 저장 실패: {session_dir} ({successful_files}/{total_files} 파일 성공)"
                )

        except Exception as e:
            self.logger.error(f"결과 저장 중 오류: {e}")

    def _save_results_fallback(
        self, results: Dict[str, Any], output_dir: str = "results/macro_optimization"
    ):
    ...
    def _save_results_fallback(
        ...
        try:
            ...
            # JSON 파일로 저장
            file_path = os.path.join(
                output_dir, f"hyperparam_optimization_results_{timestamp}.json"
            )
            with open(file_path, "w", encoding="utf-8") as f:
                json.dump(fallback_results, f, indent=2, ensure_ascii=False)

            self.logger.info(
                f"✅ HyperparamTuner 폴백 방식으로 최적화 결과 저장 완료: {file_path}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="글로벌 매크로 데이터 수집 및 하이퍼파라미터 튜닝"
    )
    parser.add_argument(
        "--mode",
        choices=["collect", "optimize"],
        default="collect",
