...
class Cleaner:
    ...
    def clean_folders(self, folders: List[str]) -> bool:
        ...
        if success_count == len(folders):
            ...
            self.logger.info(
                f"ℹ️ 폴더 정리 완료 ({success_count}/{len(folders)}) - 일부 폴더는 존재하지 않음"
            )
    ...
    def create_folders(self, folders: List[str]) -> bool:
        ...
        if success_count == len(folders):
            ...
            self.logger.warning(
                f"⚠️ 일부 폴더 생성 실패 ({success_count}/{len(folders)})"
            )
            return False
    ...
    def clean_and_recreate_folders(self, folders: List[str]) -> bool:
        ...
        if create_success:
            self.logger.info(
                f"✅ 폴더 정리 및 재생성 완료 (정리: {clean_success_count}/{len(folders)}, 생성: 성공)"
            )
            return True
        else:
    ...
    def clean_all_folders(
        self, data_dir: str = "data", log_dir: str = "log", results_dir: str = "results"
    ) -> bool:
    ...
    def clean_and_recreate_folders_legacy(
        self, data_dir: str = "data", log_dir: str = "log", results_dir: str = "results"
    ) -> bool:
        ...
        if create_success:
            self.logger.info("✅ 폴더 생성 완료")
            return True
        else:
            self.logger.error("❌ 폴더 생성 실패")
            return False
...
def main():
    ...
    parser.add_argument(
        "--folders", nargs="+", help="처리할 폴더 목록 (여러 폴더 지정 가능)"
    )
