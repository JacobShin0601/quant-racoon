...
class RLMFRegimeAdaptation:
    ...
    def __init__(
        ...
        self.logger.setLevel(logging.WARNING)  # 불필요한 로그 억제
        self.logger.setLevel(logging.WARNING)  # 불필요한 로그 억제
    ...
    def _load_feedback_history(self):
        ...
        try:
            if os.path.exists(self.feedback_file):
                ...
                self.logger.info(
                    f"피드백 히스토리 로드 완료: {len(self.feedback_history)}개"
                )
    ...
    def record_prediction(
        ...
        prediction_record = {
            ...
            "rf_top_prediction": (
                max(rf_probabilities.items(), key=lambda x: x[1])[0]
                if rf_probabilities
                else None
            ),
        }
    ...
    def calculate_rf_comparison_feedback(
        self, actual_returns: pd.Series, lookback_days: int = 5
    ) -> Dict[str, float]:
        ...
        try:
            ...
            for i, pred_record in enumerate(recent_predictions):
                if i < len(actual_direction):
                    ...
                    if rf_pred in ["trending_up", "TRENDING_UP"] and actual_dir > 0:
                        ...
                        rf_pred in ["trending_down", "TRENDING_DOWN"] and actual_dir < 0
                    ):
                        correct_predictions += 1
            ...
            if total_predictions > 0:
                feedback["rf_prediction_accuracy"] = (
            ...
            if confidence_accuracy_pairs:
                ...
                feedback["rf_confidence_alignment"] = (
                    max(0.0, correlation) if not np.isnan(correlation) else 0.5
                )
            ...
            if prob_calibration_scores:
                feedback["rf_probability_calibration"] = np.mean(
                    prob_calibration_scores
                )
            ...
            if len(recent_predictions) > 1:
                ...
                for i in range(1, len(recent_predictions)):
                    if (
                        recent_predictions[i]["rf_top_prediction"]
                        != recent_predictions[i - 1]["rf_top_prediction"]
                    ):
                        regime_changes += 1
    ...
    def calculate_market_feedback(
        ...
        try:
            ...
            return_alignment = 1.0 - min(
                1.0, abs(expected_return - actual_return) / tolerance
            )
            ...
            vol_accuracy = 1.0 - min(
                1.0, abs(predicted_volatility - actual_volatility) / vol_tolerance
            )
            ...
            # TRENDING 예측의 경우 적당한 변동성에 보너스
                ...
                prediction in ["TRENDING_UP", "TRENDING_DOWN"]
                and 0.06 <= actual_volatility <= 0.14
            ):
                vol_accuracy = min(1.0, vol_accuracy + 0.2)
    ...
    def _calculate_macro_consistency(
        self, prediction: str, macro_data: Dict[str, pd.DataFrame]
    ) -> float:
        """매크로 데이터와 예측의 일관성 계산"""
        try:
            ...
            if "^vix_data" in macro_data and not macro_data["^vix_data"].empty:
                ...
                if prediction == "VOLATILE" and current_vix > 25:
                    ...
                    prediction in ["TRENDING_UP", "TRENDING_DOWN"] and current_vix < 20
                ):
    ...
    def update_adaptation_weights(self, feedback: Dict[str, float]):
        ...
        try:
            ...
            self.feedback_history.append(
                {"timestamp": datetime.now(), "feedback": feedback.copy()}
            )
            ...
            if len(self.feedback_history) >= 3:
                recent_performance = np.mean(
                    [
                        f["feedback"]["prediction_accuracy"]
                ...
                for key in self.adaptation_weights:
                    ...
                    self.adaptation_weights[key] = max(
                        0.1, self.adaptation_weights[key]
                    )

        except Exception as e:
            self.logger.warning(f"적응 가중치 업데이트 중 오류: {e}")

    def calculate_statistical_arbitrage_signal(
        ...
        try:
            ...
            for metric, config in self.key_metrics.items():
                ...
                if (
                    data_key
                    and data_key in macro_data
                    and not macro_data[data_key].empty
                ):
            ...
            return {
                ...
                "direction": (
                    ...
                    else "BEARISH" if overall_signal < -0.1 else "NEUTRAL"
                ),
            }
    ...
    def get_adaptation_status(self) -> Dict[str, Any]:
        ...
        recent_performance = np.mean(
            [f["feedback"]["prediction_accuracy"] for f in self.feedback_history[-5:]]
        )
