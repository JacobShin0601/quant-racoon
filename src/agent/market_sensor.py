...
from actions.global_macro import (
    GlobalMacroDataCollector, MacroSectorAnalyzer, 
    HyperparamTuner, MarketRegimeValidator,
    MarketRegime, MarketCondition, MacroAnalysis
)
...
class MarketSensor:
    ...
    def _load_cached_data(self) -> Tuple[pd.DataFrame, Dict[str, pd.DataFrame], Dict[str, pd.DataFrame]]:
        """캐시된 매크로 데이터 로드"""
        try:
            # 가장 최근 캐시 파일 찾기
            ...
            # 세션별 캐시 디렉토리 찾기
            session_dirs = [d for d in os.listdir(cache_dir) if os.path.isdir(os.path.join(cache_dir, d))]
            if not session_dirs:
                return pd.DataFrame(), {}, {}
            
            # 가장 최근 세션 찾기
            latest_session = max(session_dirs, key=lambda x: os.path.getctime(os.path.join(cache_dir, x)))
            ...
            for file in os.listdir(session_path):
                if file.endswith('.csv') and not file.startswith('spy_') and not file.endswith('_sector.csv'):
                    symbol = file.replace('.csv', '')
                    file_path = os.path.join(session_path, file)
                    macro_data[symbol] = pd.read_csv(file_path, index_col=0, parse_dates=True)
            ...
            for file in os.listdir(session_path):
                if file.endswith('_sector.csv'):
                    ...
                    file_path = os.path.join(session_path, file)
                    sector_data[sector] = pd.read_csv(file_path, index_col=0, parse_dates=True)
            ...
            for optimization_dir in optimization_dirs:
                ...
                for item in os.listdir(optimization_dir):
                    ...
                    if os.path.isdir(item_path):
                        ...
                        performance_file = os.path.join(item_path, "performance_summary.json")
                        
                        if os.path.exists(best_params_file):
                            file_time = os.path.getctime(best_params_file)
                            
                            # 캐시 유효성 검사
                            if (datetime.now() - datetime.fromtimestamp(file_time)).days <= self.cache_days:
                                # JSON 파일 유효성 검사 및 로드
                                try:
                                    ...
                                    if os.path.exists(performance_file):
                                        ...
                                        temp_test_performance = performance_data.get('test_performance', {})
                                        
                                        # 유효한 파일인 경우에만 업데이트
                                        if temp_best_params and temp_test_performance and file_time > latest_time:
                                            latest_time = file_time
                                            latest_file = best_params_file
                                            best_params = temp_best_params
                                            test_performance = temp_test_performance
                                            self.logger.info(f"best_params.json 로드 성공: {len(best_params)}개 파라미터")
                                            self.logger.info(f"performance_summary.json 로드 성공: {len(test_performance)}개 지표")
                                            self.logger.info(f"Sharpe Ratio: {test_performance.get('sharpe_ratio', 'N/A')}")
                                    else:
                                        self.logger.warning(f"performance_summary.json 파일이 없습니다: {performance_file}")
                                except Exception as e:
                ...
                optimization_files = [f for f in os.listdir(optimization_dir) 
                                    if f.endswith('.json') and 
                                    ('optimization_results' in f or 'hyperparam_optimization' in f)]
                for file in optimization_files:
                    ...
                    if (datetime.now() - datetime.fromtimestamp(file_time)).days <= self.cache_days:
                        try:
                            ...
                            if 'best_params' in optimization_data and optimization_data['best_params']:
                                temp_best_params = optimization_data.get('best_params', {})
                                temp_test_performance = optimization_data.get('test_performance', {})
                                ...
                                if file_time > latest_time and temp_best_params:
                                    ...
                                    best_params = temp_best_params
                                    test_performance = temp_test_performance
                                    self.logger.info(f"폴백 최적화 결과 파일 로드: {file}")
                                    self.logger.info(f"파라미터 수: {len(best_params)}")
                                    self.logger.info(f"최고 점수: {optimization_data.get('best_value', 'N/A')}")
                                    self.logger.info(f"Trial 수: {optimization_data.get('n_trials', 'N/A')}")
                                    if test_performance:
                                        self.logger.info(f"Test 성과: {list(test_performance.keys())}")
                        except Exception as e:
    ...
    def _copy_data_to_macro_dir(self):
        ...
        try:
            ...
            self.logger.info(f"✅ 매크로 데이터 복사 완료 ({csv_count}개 CSV 파일 + 메타데이터)")
    ...
    def run_basic_analysis(
        ...
        try:
            ...
            # 2-1. 상세 매크로 환경 분석
            detailed_macro_analysis = self.macro_analyzer.analyze_macro_environment(macro_data)
            
            # 2-2. 섹터 로테이션 분석
            sector_analysis = self.macro_analyzer.analyze_sector_rotation(sector_data)
            
            # 2-3. 섹터 추천 생성
            sector_recommendations = self.macro_analyzer.generate_sector_recommendations(
                macro_analysis.market_condition, 
                sector_analysis
            )
            ...
            if self.use_cached_optimization:
                self.optimal_params, self.optimization_performance = self._load_cached_optimization()
                
                if not self.optimal_params:
                    raise ValueError("캐시된 최적화 결과가 없습니다. 새로 최적화하려면 --use-cached-optimization 옵션을 제거하세요.")
            else:
            ...
            stat_arb_confidence = rlmf_analysis.get('statistical_arbitrage', {}).get('confidence', 0.5)
            rlmf_confidence = np.mean(list(rlmf_analysis.get('market_feedback', {}).values())) if rlmf_analysis.get('market_feedback') else 0.5
            cross_val_confidence = self._calculate_cross_validation_confidence(spy_data, regime_classification)
            ...
            confidence = comprehensive_confidence.get('adjusted_confidence', base_confidence)

            # 5. 검증 수행
            validation_results = self._perform_validation(spy_data, macro_data, regime_classification)
            ...
            result = MarketAnalysisResult(
                ...
                enhanced_recommendations=self._generate_basic_recommendations(macro_analysis, current_regime),
        ...
        try:
            ...
            if self.llm_api_system:
                ...
                llm_api_insights = self._perform_llm_api_analysis(
                    basic_result.macro_analysis, basic_result.current_regime, analysis_results
                )
    ...
    def _print_detailed_regime_analysis(self, classification_result: Dict[str, Any]):
        ...
        if 'scores' in classification_result:
            ...
            for col in scores.columns:
                if col in scores:
                    ...
                    print(f"  • {col}: 평균={mean_score:.3f}, 표준편차={std_score:.3f}, 범위=[{min_score:.3f}, {max_score:.3f}]")
        ...
        if 'probabilities_series' in classification_result:
            ...
            for regime, probs in prob_series.items():
                ...
                print(f"  • {regime.upper()}: 평균={mean_prob:.1%}, 표준편차={std_prob:.1%}, 범위=[{min_prob:.1%}, {max_prob:.1%}]")
    ...
    def _classify_market_regime_with_optimal_params(
        ...
        try:
            ...
            if self.use_random_forest and self.rf_model:
                ...
                classification_result = self.hyperparam_tuner._classify_market_regime_with_probabilities(
    ...
    def _classify_with_random_forest(
        ...
        try:
            ...
            if self.retrain_rf_model:
                ...
                # 모델 학습
                training_results = self.rf_model.train_model(training_data, save_model=True)
                self.logger.info(f"모델 학습 완료 - 테스트 정확도: {training_results['test_score']:.4f}")
            else:
                # 저장된 모델 로드 시도
                try:
            ...
            # 현재 시장 상태 확률 예측
            probabilities = self.rf_model.get_current_market_probabilities(self.data_dir)
            ...
            current_regime_enum = regime_mapping.get(current_regime, MarketRegime.UNCERTAIN)
    ...
    def _perform_enhanced_quant_analysis(
        ...
        try:
            ...
            regime_detection = self.regime_detector.detect_regime_shifts(spy_data, macro_data)
            regime_stability = self.regime_detector.analyze_regime_stability(spy_data)
            regime_persistence = self.regime_detector.calculate_regime_persistence(spy_data)
            ...
            if hasattr(self, 'last_prediction') and hasattr(self, 'last_returns'):
                ...
                # RF 모델 비교 피드백 추가
                rf_comparison_feedback = self.rlmf_adaptation.calculate_rf_comparison_feedback(
                ...
                market_feedback = self.rlmf_adaptation.calculate_market_feedback(
                    current_regime.value if hasattr(current_regime, 'value') else str(current_regime),
            ...
            stat_arb_signals = self.rlmf_adaptation.calculate_statistical_arbitrage_signal(macro_data)
            ...
            rlmf_confidence = np.mean(list(market_feedback.values())) if market_feedback else 0.5
            cross_val_confidence = self._calculate_cross_validation_confidence(spy_data, classification_result)
            ...
            # 5. 고급 Quant 지표 계산
            advanced_indicators = self._calculate_advanced_quant_indicators(spy_data, macro_data, optimal_params)
            
            # 6. 결과 통합
            enhanced_analysis = {
                # Regime 감지 결과
                'regime_detection': {
                    'shift_detected': regime_detection.get('regime_shift_detected', False),
                    'shift_score': regime_detection.get('shift_score', 0.0),
                    'stability_score': regime_stability.get('stability_score', 0.5),
                    'persistence_score': regime_persistence.get('persistence_score', 0.5),
                    'expected_duration': regime_persistence.get('expected_duration', 'unknown')
                },
                ...
                'quant_score': self._calculate_quant_score(
                    classification_result, regime_detection, market_feedback, 
                    stat_arb_signals, comprehensive_confidence
                )
            }
    ...
    def _calculate_cross_validation_confidence(self, spy_data: pd.DataFrame, classification_result: Dict[str, Any]) -> float:
        """교차 검증 기반 신뢰도 계산"""
        try:
            if len(spy_data) < 60:
                return 0.5
            ...
            # 변동성 일관성
            volatility_consistency = 1.0 - (returns.std() / returns.mean()) if returns.mean() != 0 else 0.5
            ...
            # 종합 신뢰도
            cross_val_confidence = (volatility_consistency + trend_consistency + prediction_consistency) / 3
    ...
    def _calculate_buyhold_performance(self, spy_data: pd.DataFrame) -> Dict[str, float]:
        """Buy & Hold 성과 계산"""
        try:
            ...
            buyhold_sharpe = (excess_return * 252) / (std_return * np.sqrt(252)) if std_return > 0 else 0.0
    ...
    def _calculate_advanced_quant_indicators(self, spy_data: pd.DataFrame, macro_data: Dict[str, pd.DataFrame], optimal_params: Dict[str, Any]) -> Dict[str, Any]:
        """고급 Quant 지표 계산"""
        try:
            indicators = {}
            ...
            indicators['volatility_structure'] = {
                ...
                'vol_regime': 'high' if returns.std() > returns.rolling(60).std().mean() else 'normal'
            }
            ...
            if '^VIX' in macro_data and not macro_data['^VIX'].empty:
                ...
                indicators['correlation_analysis'] = {
                    ...
                    'correlation_regime': 'negative' if correlation < -0.3 else 'positive' if correlation > 0.3 else 'neutral'
                }
            ...
            if 'macd' in spy_data.columns and 'macd_signal' in spy_data.columns:
                ...
                macd_momentum = (macd.iloc[-1] - macd_signal.iloc[-1]) / abs(macd_signal.iloc[-1]) if abs(macd_signal.iloc[-1]) > 0 else 0
            ...
            if 'volume' in spy_data.columns:
                ...
                indicators['volume_analysis'] = {
                    ...
                    'volume_regime': 'high' if volume.tail(20).mean() > volume.tail(60).mean() * 1.2 else 'normal'
                }
            
            # 5. 시장 미세구조 분석
            indicators['market_microstructure'] = {
                'bid_ask_spread_proxy': returns.abs().mean(),  # 간단한 스프레드 프록시
                'price_efficiency': 1.0 - abs(returns.autocorr()) if len(returns) > 1 else 0.5,
    ...
    def _calculate_quant_score(self, classification_result: Dict[str, Any], regime_detection: Dict[str, Any], 
                              market_feedback: Dict[str, float], stat_arb_signals: Dict[str, Any], 
                              comprehensive_confidence: Dict[str, Any]) -> float:
        """종합 Quant 점수 계산"""
        try:
            scores = []
            ...
            # 5. 종합 신뢰도
            comprehensive_conf = comprehensive_confidence.get('adjusted_confidence', 0.5)
    ...
    def _perform_validation(
        ...
        try:
            ...
            validation_results = {
                ...
                'regime_consistency': {
                    'confidence_threshold_met': regime_classification['confidence'] > 0.6,
                    'probability_sum': sum(regime_classification['probabilities'].values())
                }
            }
    ...
    def _perform_rlmf_analysis(
        ...
        try:
            ...
            if spy_data.empty or not macro_data:
                self.logger.warning("매크로 데이터를 찾을 수 없어 RLMF 분석을 건너뜁니다.")
                return {
    ...
    def _perform_confidence_analysis(
        ...
        try:
            ...
            # 종합 신뢰도 계산
            confidence_result = self.confidence_system.calculate_comprehensive_confidence(
    ...
    def _calculate_final_confidence(
        ...
        try:
            ...
            adjusted_confidence = confidence_result.get("adjusted_confidence", base_confidence)
    ...
    def _generate_basic_recommendations(
        ...
        try:
            ...
            if market_condition == MarketCondition.RECESSION_FEAR:
                recommendations["risk_warnings"].append("경기침체 우려 - 현금 비중 확대 고려")
                recommendations["key_actions"].append("국채 비중 확대")
            elif market_condition == MarketCondition.INFLATION_FEAR:
                recommendations["risk_warnings"].append("인플레이션 우려 - 실물자산 비중 확대")
            ...
            # 매크로 분석의 추천 정보 추가
            if hasattr(macro_analysis, 'recommendations') and macro_analysis.recommendations:
    ...
    def _generate_enhanced_recommendations(
        ...
        try:
            ...
            # Regime 전환 감지 기반 추천
            if regime_detection.get("regime_shift_detection", {}).get("regime_shift_detected", False):
                recommendations["key_considerations"].append(
            ...
            # 신뢰도 기반 포지션 사이징 조정
            final_confidence = basic_result.final_confidence.get("final_confidence", 0.5)
        ...
        try:
            ...
            result_dict = {
                ...
                "macro_analysis": {
                    ...
                    "sector_rotation": {k: v.value for k, v in result.macro_analysis.sector_rotation.items()},
    ...
    def _print_basic_summary(self, result: MarketAnalysisResult):
        ...
        base_confidence = result.classification_result.get('confidence', 0.5) if result.classification_result else 0.5
        print(f"  기본 신뢰도: {base_confidence:.3f}")
        
        # 다층 신뢰도 시스템 결과
        if result.confidence_analysis:
            confidence_analysis = result.confidence_analysis
            adjusted_confidence = confidence_analysis.get('adjusted_confidence', base_confidence)
        ...
        enhanced_keys = ['quant_score', 'regime_detection', 'rlmf_analysis', 'confidence_analysis', 'advanced_indicators']
        
        # MarketAnalysisResult 객체에서 classification_result 접근
        classification_result = result.classification_result if result.classification_result else {}
        has_enhanced = any(key in classification_result and classification_result[key] for key in enhanced_keys)
        
        if has_enhanced:
            # Quant 점수
            if 'quant_score' in classification_result and classification_result['quant_score']:
                print(f"  🎯 Quant 점수: {classification_result['quant_score']:.3f}")
            
            # Regime 감지 결과
            if 'regime_detection' in classification_result and classification_result['regime_detection']:
                regime_det = classification_result['regime_detection']
                print(f"  🔄 Regime 안정성: {regime_det.get('stability_score', 0):.3f}")
                print(f"  ⏱️ 예상 지속기간: {regime_det.get('expected_duration', 'unknown')}")
                if regime_det.get('shift_detected', False):
                    print(f"  ⚠️ Regime 변화 감지! (점수: {regime_det.get('shift_score', 0):.3f})")
            
            # RLMF 분석 결과
            if 'rlmf_analysis' in classification_result and classification_result['rlmf_analysis']:
                ...
                if 'statistical_arbitrage' in rlmf:
                    ...
                    print(f"  📈 Statistical Arbitrage: {direction} (강도: {signal_strength:.3f}, 신뢰도: {confidence:.3f})")
                    ...
                    if individual_signals:
                        ...
                        for metric, signal_data in individual_signals.items():
                            ...
                            print(f"      • {metric}: {signal:+.1f} (수익률: {ret:+.1%}, 가중치: {weight:.0%})")
            
            # 신뢰도 분석 결과
            if 'confidence_analysis' in classification_result and classification_result['confidence_analysis']:
            ...
            # 고급 지표
            if 'advanced_indicators' in classification_result and classification_result['advanced_indicators']:
                ...
                if 'momentum_structure' in indicators:
                    ...
                    print(f"  🚀 모멘텀 정렬: {mom_alignment} (점수: {momentum_score:.3f})")
                    ...
                    print(f"    └─ 단기: {short_mom:+.1%}, 중기: {medium_mom:+.1%}, 장기: {long_mom:+.1%}, 가속도: {accel:+.1%}")
        ...
        if hasattr(self, 'use_random_forest') and self.use_random_forest:
            ...
            if result.probabilities:
                ...
                if quant_regime.upper() == ml_regime.upper():
                    ...
                    print(f"  ⚠️ Quant와 ML 예측 차이: Quant={quant_regime}, ML={ml_regime}")
        else:
            print("\n📈 체제별 확률 (Quant 기반):")
            if result.probabilities:
                ...
                if sector_data:
                    ...
                    if sector_analysis:
                        ...
                        for sector, strength in sector_analysis.items():
                            strength_emoji = "🚀" if strength.value == 'LEADING' else "📉" if strength.value == 'LAGGING' else "➡️"
        ...
        # 섹터별 상세 정보 (가능한 경우)
        if hasattr(result.macro_analysis, 'sector_performance') and result.macro_analysis.sector_performance:
            print("\n📈 섹터별 성과:")
            for sector, perf in result.macro_analysis.sector_performance.items():
                if isinstance(perf, dict):
                    return_val = perf.get('return', 0)
                    volatility = perf.get('volatility', 0)
                    print(f"  • {sector}: 수익률 {return_val:+.2%}, 변동성 {volatility:.2%}")
        ...
        if result.macro_analysis.sector_rotation:
            ...
            # 섹터 로테이션 패턴 분석
            leading_count = sum(1 for s in result.macro_analysis.sector_rotation.values() if s.value == 'LEADING')
            lagging_count = sum(1 for s in result.macro_analysis.sector_rotation.values() if s.value == 'LAGGING')
            ...
            if total_sectors > 0:
                ...
                print(f"\n📊 섹터 로테이션 패턴:")
                print(f"  • 선도 섹터 비율: {leading_ratio:.1%} ({leading_count}/{total_sectors})")
                print(f"  • 후행 섹터 비율: {lagging_ratio:.1%} ({lagging_count}/{total_sectors})")
                
                if leading_ratio > 0.5:
                    print(f"  🎯 시장 전망: 순환적 섹터 선호 (강세장 신호)")
                elif lagging_ratio > 0.5:
                    print(f"  ⚠️ 시장 전망: 방어적 섹터 선호 (약세장 신호)")
                else:
                    print(f"  ➡️ 시장 전망: 혼재된 신호 (횡보장 가능성)")
        ...
        if result.enhanced_recommendations:
            ...
            if current_regime in regime_strategy_fit:
                ...
                if primary_strategy == recommended_strategy:
                    ...
                    print(f"⚠️ 전략 적합성: 현재 시장 체제({current_regime})에는 {recommended_strategy} 전략이 더 적합할 수 있음")
            ...
            # 섹터 추천
            if 'overweight_sectors' in rec and rec['overweight_sectors']:
                print(f"\n📈 과중 배치 섹터:")
                for i, sector in enumerate(rec['overweight_sectors'], 1):
                    print(f"  {i}. {sector}")
            
            if 'underweight_sectors' in rec and rec['underweight_sectors']:
                print(f"\n📉 과소 배치 섹터:")
                for i, sector in enumerate(rec['underweight_sectors'], 1):
                    print(f"  {i}. {sector}")
            ...
            # 섹터 로테이션 추천
            if 'sector_rotation' in rec and rec['sector_rotation']:
                print(f"\n🔄 섹터 로테이션:")
                for i, rotation in enumerate(rec['sector_rotation'], 1):
                    print(f"  {i}. {rotation}")
    ...
    def _print_enhanced_summary(self, result: MarketAnalysisResult):
        ...
        # Quant vs ML 비교
        if hasattr(self, 'use_random_forest') and self.use_random_forest and result.probabilities:
            quant_regime = result.current_regime.value
            sorted_probs = sorted(result.probabilities.items(), key=lambda x: x[1], reverse=True)
            ml_regime = sorted_probs[0][0].replace('_', ' ').title()
        ...
        print(f"주요 전략: {result.enhanced_recommendations.get('primary_strategy', 'N/A')}")
        print(f"포지션 사이징: {result.enhanced_recommendations.get('position_sizing', 'N/A')}")
...
def main():
    ...
    parser.add_argument("--analysis-type", choices=["basic", "enhanced"], default="enhanced",
                       help="분석 유형 선택 (기본값: enhanced)")
    parser.add_argument("--output-dir", default="results/macro/enhanced",
                       help="결과 출력 디렉토리")
    parser.add_argument("--use-cached-data", action="store_true",
                       help="저장된 매크로 데이터 사용")
    parser.add_argument("--use-cached-optimization", action="store_true",
                       help="저장된 최적화 결과 사용")
    ...
    parser.add_argument("--retrain-rf-model", action="store_true",
                       help="Random Forest 모델 재학습")
    ...
    print(f"캐시 설정: 데이터={args.use_cached_data}, 최적화={args.use_cached_optimization}")
